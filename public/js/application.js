...
(function($, undefined) {
	...
	function buildOptions($cont, $slides, els, options, o) {
		...
		if (reshape) {
			...
			if (opts.containerResize && maxw > 0 && maxh > 0) $cont.css({ width: maxw + 'px', height: maxh + 'px' });
		...
		$slides.each(function() {
			...
			this.cycleW =
				opts.fit && opts.width ? opts.width : $el.width() || this.offsetWidth || this.width || $el.attr('width') || 0;
	...
	function go(els, opts, manual, fwd) {
		...
		if ((manual || !p.cyclePause) && opts.nextSlide != opts.currSlide) {
			...
			opts.busy = 1;
			if (
				opts.fxFn // fx function provided?
			)
				opts.fxFn(curr, next, opts, after, fwd, manual && opts.fastOnEvent);
			else if (
				$.isFunction($.fn.cycle[opts.fx]) // fx plugin ?
			)
		...
		function queueNext() {
			...
			if (opts.timeout && !opts.continuous) {
				...
				opts.continuous &&
				p.cyclePause // continuous shows work off an after callback, not this timer logic
			)
				ms = 10;
			if (ms > 0)
	...
	function getTimeout(curr, next, opts, fwd) {
		if (opts.timeoutFn) {
			...
			while (
				opts.fx != 'none' &&
				t - opts.speed < 250 // sanitize timeout
			)
				t += opts.speed;
